...
export class SelectHandler {
  constructor (el) {
    ...
    // g 签 需要监听子元素 lmj
    this.observerOption = this.observerOption()
  }
  ...
  init () {
    ...
    this.observer.observe(this.el.node, this.observerOption)
  }
  ...
  updatePoints () {
    ...
    const screenCTM = this.el.type === 'svg' ? this.el.findOne('g').screenCTM() : this.el.screenCTM()
    const fromShapeToUiMatrix = parent.screenCTM().inverseO().multiplyO(screenCTM)
  ...
  observerOption() {
    if (this.el.type === 'g') {
      return { childList: true, attributes: true, subtree: true }
    }else return  { attributes: true }
  }
}
